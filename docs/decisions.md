...
type generator. A small number — notably `stateless-exchanges-air-search` —
ship as Swagger 2.0, which `openapi-typescript` does not accept.
...
- **Simpler than dual-format support** in `scripts/generate-types.mjs`.
  Swagger 2.0 specs are rare here (one as of this writing), so a script
  branch isn't worth its complexity yet. If we accumulate several, promote
  the conversion into the script.
