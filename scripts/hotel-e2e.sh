...
#                               from step 4.
#   Step 6: get-booking       — SKIPPED. Blocked on CERT OAuth
#                               entitlement (UNAUTHORIZED_ACCESS).
#   Step 7: cancel-booking    — runs against Sabre CERT with --cancel-all
...
#
# Step 5 guest identity is generated by scripts/lib/random-person.sh;
...
#   --skip-cancel                  Leave the PNR live (skip step 7)
...
if [[ -z "$BOOKING_KEY" ]]; then
  ...
  BOOK_FILE=$(mktemp)
  trap 'cleanup; rm -f "$TMP_ERR" "$SEARCH_FILE" "$AVAIL_FILE" "$DETAILS_FILE" "$PRICE_FILE" "$BOOK_FILE"' EXIT
...
skip 6 "get-booking (verify hotel)" "UNAUTHORIZED_ACCESS on this EPR; Sabre support ticket pending"
...
if [[ -z "$PNR_LOCATOR" ]]; then
  skip 7 "cancel-booking" "no PNR locator to cancel"
elif [[ "$SKIP_CANCEL" == "1" ]]; then
  skip 7 "cancel-booking" "--skip-cancel supplied; leaving PNR live"
else
  step 7 "cancel-booking (cancelAll)"
  CANCEL_FILE=$(mktemp)
  trap 'cleanup; rm -f "$TMP_ERR" "$SEARCH_FILE" "$AVAIL_FILE" "$DETAILS_FILE" "$PRICE_FILE" "$BOOK_FILE" "$CANCEL_FILE"' EXIT
...
echo "               4 (hotel-price-check), 5 (book-hotel), 7 (cancel-booking)."
echo "Skipped: 6 (get-booking) — blocked on Sabre EPR entitlement."
