...
/** A single airline alliance as returned by the Airline Alliance Lookup v1 service. */
export interface AirlineAlliance {
  /** Alliance group code (e.g., `*A`, `*O`, `*S`). */
  code: string;
  /** Alliance name (e.g., "Star Alliance", "oneworld", "SkyTeam"). */
  name: string;
  /**
   * Two-character IATA codes of the member airlines. Sabre's response only
   * includes the codes — to look up names, pass these to
   * {@link AirlineLookupV1Service.lookup}.
   */
  memberAirlineCodes: readonly string[];
}
